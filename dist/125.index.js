...
const catalog_model_1 = __webpack_require__(70606);
const backstage_plugin_blockchainradar_common_1 = __webpack_require__(83208);
...
function generateContractAccessKeyMetrics(collector, backstageUrl) {
    const retiredUsers = collector
        .getUserEntities()
        .filter((user) => user.metadata.tags && user.metadata.tags.includes("retired"));
    const accessKeysPerContract = collector
        ...
        const spec = JSON.parse(JSON.stringify(key.spec));
        const { owner } = spec;
        if (retiredUsers.find((user) => `user:default/${user.metadata.name}` === owner)) {
            return { ...acc };
        }
        return {
            ...acc,
