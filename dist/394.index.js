...
class UnknownCollector {
    ...
    collectEntities(opts = {}) {
        const accessKeyCollector = new access_key_collector_1.AccessKeyCollector(this.entities, opts);
        const multisigCollector = new multisigs_collector_1.MultisigsCollector(this.entities, opts);
        const rbacCollector = new rbac_collector_1.RbacCollector(this.entities, opts);
            ...
            return [
                ...acc,
                {
                    ...
                    components: system.components
                        ...
                        return {
                            ...component,
                            multisigs: component.multisigs.filter((m) => m.signers.some((info) => info.signer.metadata.namespace === "stub")),
                        };
                    })
                        .filter((component) => component.multisigs.some((multisig) => multisig.signers.some((info) => info.signer.metadata.namespace === "stub"))),
                },
            ];
        }, [])
            ...
            return [
                ...acc,
                {
                    ...
                    components: system.components
                        ...
                        return {
                            ...component,
                            contracts: component.contracts.filter((contract) => contract.roles.some((info) => info.members.some((m) => m.member.metadata.namespace === "stub"))),
                        };
                    })
                        .filter((component) => component.contracts.some((contract) => contract.roles.some((info) => info.members.some((m) => m.member?.metadata.namespace === "stub")))),
                },
            ];
        }, [])
            .filter((system) => system.components.length > 0);
        const unknownAccessKeys = accessKeyCollector.systemComponents
            ...
            return [
                ...
                {
                    ...system,
                    components: system.components.filter((component) => component.contracts.some((contract) => contract.keys.some((key) => key.key.metadata.namespace === "stub"))),
                },
            ];
        }, [])
            .filter((system) => system.components.length > 0);
