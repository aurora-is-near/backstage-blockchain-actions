...
type Mocktokit = jest.MockInstance<any, any[]>;
...
jest.mock("@actions/github", () => ({
  context: {
    repo: { repo: "rainbow-bridge", owner: "aurora-is-near" },
    issue: { number: 123 },
    payload: {
      pull_request: {
        number: 124,
...
(octokit.pulls.listFiles as unknown as Mocktokit).mockImplementation(
  async ({ page }) => ({
    data:
      page === 1
        ? [
